...
def embed_setup(bot, banner, page):
    ...
    if page == 1:
        embed = discord.Embed(title="> 📌 OP.GG E-Sports 봇 서비스 가이드", description="```서비스 소개 페이지```", color=colorMap['red'])
        embed.add_field(name="▫️ 소개", value=" 본 디스코드 봇은 [OP.GG E-Sports](https://esports.op.gg/) 서비스의 공식 디스코드 봇이며, **리그 오브 레전드**의 e스포츠 리그 관련 기능을 제공합니다.", inline=False)
        embed.add_field(name="▫️ 기능", value=" 기본적으로 리그 오브 레전드의 e스포츠 일정 조회, 경기 알림 등을 제공하지만 더욱 재밌게 승부 예측 미니게임도 즐길 수 있어요.", inline=False)
        embed.add_field(name="▫️ 서포트 서버", value=" [OP.GG 서비스 서포트 서버](https://discord.com/invite/opgg/)에 입장하여 유저들과 소통하고 OP.GG E-Sports 봇과 다른 서비스들의 정보를 확인해보세요!", inline=False)
        embed.add_field(name="▫️ 봇 초대", value=f" 지금 바로 OP.GG E-Sports 봇을 [초대]({inviteURL})해서 사용해보세요. 😆", inline=False)
        ...
        embed.add_field(name="/가이드", value="OP.GG E-Sports 봇의 가이드를 전송해요.", inline=False)
        ...
        embed.add_field(name="/피드백", value="[Beta] OP.GG E-Sports 봇의 피드백 또는 의견 제안을 제출할 수 있어요.", inline=False)

    elif page == 4:
        embed = discord.Embed(title="> 📌 OP.GG E-Sports 봇 서비스 가이드", description="```관리자 명령어 페이지```", color=colorMap['red'])
...
class GuideCMD(commands.Cog):
    ...
    @slash_command(
        name="가이드",
        description="OP.GG E-Sports 봇의 가이드를 확인해보세요.",
    )
