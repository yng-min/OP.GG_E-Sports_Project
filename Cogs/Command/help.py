...
class HelpBUTTON(discord.ui.View):
    ...
    def add_button(self):
        _prev = discord.ui.Button(label="◀️", style=discord.ButtonStyle.blurple, custom_id="prev")
        _next = discord.ui.Button(label="▶️", style=discord.ButtonStyle.blurple, custom_id="next")
        ...
        async def _prev_callback(interaction: discord.Interaction):
            ...
            if self.page == 1:
                ...
                embed.set_footer(text="TIP: 아래 버튼을 눌러 페이지를 넘길 수 있어요.", icon_url=self.bot.user.display_avatar.url)
                ...
                embed.add_field(name="/가입", value="서비스를 이용을 위한 기본 설정을 진행해요.", inline=False)
                embed.add_field(name="/탈퇴", value="서비스를 탈퇴하고 데이터를 삭제해요.", inline=False)
                embed.add_field(name="/프로필", value="서비스 내의 유저 프로필 정보를 조회해요.", inline=False)
                embed.add_field(name="/리그 일정", value="리그 오브 레전드 e스포츠 리그 일정을 확인할 수 있어요.", inline=False)
                ...
                embed.set_footer(text="TIP: 아래 버튼을 눌러 페이지를 넘길 수 있어요.", icon_url=self.bot.user.display_avatar.url)
                embed.set_image(url=self.banner)
                embed.add_field(name="/셋업", value="서비스를 이용을 위한 기본 서버 설정을 진행해요.", inline=False)
        ...
        async def _next_callback(interaction: discord.Interaction):
            ...
            if self.page == 1:
                ...
                embed.add_field(name="/가이드", value="OP.GG Esports 봇의 가이드를 전송해요.", inline=False)
                embed.add_field(name="/가입", value="서비스를 이용을 위한 기본 설정을 진행해요.", inline=False)
                embed.add_field(name="/탈퇴", value="서비스를 탈퇴하고 데이터를 삭제해요.")
                embed.add_field(name="/프로필", value="서비스 내의 유저 프로필 정보를 조회해요.", inline=False)
                embed.add_field(name="/리그 일정", value="리그 오브 레전드 e스포츠 리그 일정을 확인할 수 있어요.", inline=False)
                embed.add_field(name="/피드백", value="[Beta] OP.GG Esports 봇의 피드백 또는 의견 제안을 제출할 수 있어요.", inline=False)
...
class HelpCMD(commands.Cog):
    ...
    async def on_ready(self):
        self.bot.add_view(HelpBUTTON(self.bot, self.banner))
    ...
    async def _helpCMD(self, ctx):
        ...
        embed.add_field(name="▫️ 기능", value=" ~~기본적으로 리그 오브 레전드의 e스포츠 일정 조회, 경기 알림 등을 제공하지만 더욱 재밌게 승부 예측 미니게임도 즐길 수 있어요.~~", inline=False)
        embed.add_field(name="▫️ 서포트 서버", value=" [OP.GG 서비스 서포트 서버](https://discord.com/invite/opgg/)에 입장해 보세요.")
        await ctx.respond(embed=embed, view=HelpBUTTON(self.bot, banner_image_url))
